      ...
      <div class="commentList">
        ...
        <span class="right p1">
          <div class="rightTop" v-if="item.user">
            ...
            <template v-for="userTag in userTagDictList">
              <el-tag style="height: 30px; margin-left:5px;" :key="userTag.uid"
          ...
          <div class="rightCenter ck-content" v-html="$xss(item.content, options)"></div>

          <div class="rightBottom">
            <el-link class="b1" :underline="false" @click="replyTo(item)" v-if="level < maxReplyLevel">回复</el-link>
            <el-link class="b1" :underline="false" @click="reportreport(item)">举报</el-link>
          ...
          <div class="rightCommentList">
            ...
            <CommentList class="commentStyle" :level="level + 1" :maxReplyLevel="maxReplyLevel"
                         :id="'commentStyle:' + item.uid" :comments="item.replyList"
...
export default {
  ...
  data() {
    return {
      ...
      defaultAvatar: '../../statis/icons/avatar.svg'
    };
  },
  created() {
    this.getDictList()
  ...
  methods: {
    ...
    replyTo: function (item) {
      ...
      const lists = document.getElementsByClassName("comment");
    ...
    deleteCommentList(commentList, uid) {
      if (commentList == undefined || commentList.length <= 0) {
        return;
      }
      for (let item of commentList) {
